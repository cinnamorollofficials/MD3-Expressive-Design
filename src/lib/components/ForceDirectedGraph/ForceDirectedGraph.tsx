...
export interface ForceDirectedGraphProps {
  /** Array of node objects for the graph */
  nodes: NetworkNode[];
  ...
  /** Collision radius buffer around nodes to prevent overlapping */
  collideRadius?: number;
  /** Color palette for groups (uses MD3 tokens if not provided) */
  colors?: string[];
  /** Show node text labels directly on the graph */
  showLabels?: boolean;
  /** Show group legend below graph */
  showLegend?: boolean;
  /** Enable dragging nodes to re-position */
  draggable?: boolean;
...
export function ForceDirectedGraph({
  ...
  // Simulation instance ref
  const simulationRef = useRef<d3.Simulation<NetworkNode, NetworkLink> | null>(null);
  // Cloned nodes & links references to prevent mutating original props
  ...
  // Compute unique groups for legend and color scale
  ...
  const getRadius = useCallback(
    (node: NetworkNode) => {
      ...
      if (node.val) return Math.max(4, Math.min(24, Math.sqrt(node.val) * 3));
      return nodeRadius;
    },
    [nodeRadius]
  );
  ...
  useEffect(() => {
    ...
    // Clone data so D3 mutation doesn't affect caller props directly
    ...
    // On tick update React state to re-render SVG positions
    simulation.on('tick', () => {
      setTick((t) => t + 1);
    });
  ...
  // Toggle group selection in legend
  ...
  return (
    <div className={cn(styles.root, className)} ref={containerRef} data-md3-component="force-directed-graph">
      ...
      >
              ...
              {simLinksRef.current.map((link, idx) => {
                ...
                return (
                  <line
                    ...
                    x2={targetNode.x}
                    y2={targetNode.y}
                    strokeWidth={link.value ? Math.max(1, Math.min(6, Math.sqrt(link.value))) : 1.5}
                  />
                );
              })}
            </g>
                  ...
                  >
                    ...
                    {showLabels && (
                      <text
                        ...
                        x={r + 4}
                        y={4}
                        opacity={hoveredNode || selectedGroups.size > 0 ? (isHighlighted ? 1 : 0.2) : 0.85}
                      >
        ...
        {interactive && hoveredNode && mousePos && (
          ...
          >
            ...
            {hoveredNode.val !== undefined && (
              <div className={styles.tooltipRow}>
                <span>Value:</span>
                <span className={styles.tooltipValue}>{hoveredNode.val}</span>
              </div>
            )}
          </div>
        )}
