...
function ComponentMiniVisual({ groupId, componentId, index }: { groupId: string; componentId: string; index: number }) {
  ...
  if (groupId === 'analysis') {
    return <span className={styles.miniChart} data-chart="dots" style={{ '--mini-accent': `var(--md-sys-color-${index % 3 === 0 ? 'primary' : index % 3 === 1 ? 'secondary' : 'tertiary'})` } as React.CSSProperties}><i /><i /><i /><i /><i /></span>;
  }
